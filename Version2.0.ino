...
void avanzar(){
  ...
  cambiarpotenciamotores(130,-15);
  while(contadorI<29 && contadorD<24){
...
void giroejeR(){
   ...
   while(contadorI<21 && contadorD<=19){
   digitalWrite(derecha2, LOW);
   digitalWrite(derecha1, HIGH);
   digitalWrite(izquierda1, LOW);
   digitalWrite(izquierda2, HIGH);
...
void giroejeLFASE2(){
  ...
  if(getDistanceR()<15){
    ...
    cambiarpotenciamotores(250,80);
  while(contadorI<=22 && contadorD<23){
   digitalWrite(derecha2, HIGH);
   digitalWrite(derecha1, LOW);
   digitalWrite(izquierda1, HIGH);
   digitalWrite(izquierda2, LOW);
...
void   giroejeRFASE2(){
   ...
   cambiarpotenciamotores(170,-80) ;
   while(contadorI<21 && contadorD<=20){
   digitalWrite(derecha2, LOW);
   digitalWrite(derecha1, HIGH);
   digitalWrite(izquierda1, LOW);
   digitalWrite(izquierda2, HIGH);
...
char getColor(){
        ...
        if(frecuenciaRoja>55 || frecuenciaRoja<25){
          rojo = 0;
         }
         
         // Si el valor entra en el rango, hace un map de los valores
         if (frecuenciaRoja>=25 && frecuenciaRoja<=55){
          rojo  = map(frecuenciaRoja,25,55,255,0);
         }
         delay(5);
        ...
        frecuenciaVerde = pulseIn(out, LOW);
         ...
         // Filtro para que no tome valores más grandes que los máx y mínimos
          if(frecuenciaVerde>60 || frecuenciaVerde<35){
          verde = 0;
         }
         
         // Si el valor entra en el rango, hace un map de los valores
         if (frecuenciaVerde>=35 && frecuenciaVerde<=60){
          verde = map(frecuenciaVerde,35,60,255,0);
         }
              delay(5);
        ...
        // Filtro para que no tome valores más grandes que los máx y mínimos
        if(frecuenciaAzul>130 || frecuenciaAzul<23){
          azul = 0;
         }
         
         // Si el valor entra en el rango, hace un map de los valores
         if (frecuenciaAzul>=23&& frecuenciaAzul<=130){
          azul = map(frecuenciaAzul,23,130,255,0);
         }
              delay(5);

      if (frecuenciaAzul>=45 && frecuenciaAzul<=80 && frecuenciaVerde>=22 && frecuenciaVerde<=50 && frecuenciaRoja>=16 && frecuenciaRoja<=40){
...
bool sensorbola(){
  ...
  {  
   for(int counter1=0;counter1<12;counter1++) // to receive the next 12 bits  
   {  
...
{   
  /*
    while(true){
      ...
      //calibrarcolor();
      getColor();
      Serial.print("Ultrasonico F");
      Serial.println(getDistanceF());
      
      Serial.print("Ultrasonico D");
      Serial.println(getDistanceR());
      
      Serial.print("Ultrasonico I");
